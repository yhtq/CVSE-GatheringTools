...
from bilibili_api import aid2bvid, video, bvid2aid
import capnp
import pandas as pd
...
def get_ranks(data_row) -> List[Rank]:
    list_rank = []
    if data_row["in_sv"]: list_rank.append(Rank.SV)
    if data_row["in_domestic"]: list_rank.append(Rank.DOMESTIC)
    if data_row["in_utau"]: list_rank.append(Rank.UTAU)
    return list_rank
...
async def process_batch(client: CVSE_Client, data_rows: List[Dict[str, Any]]):
    ...
    try:
        ...
        indexes = [Index_to_capnp(Index(avid=data_row['avid'], bvid=data_row['bvid']))
                   for data_row in data_rows]
        result = await asyncio.wait_for(client.lookupMetaInfo(indexes), timeout=LOOKUP_TIMEOUT)

        listExistBVid = [entry.bvid for entry in result]


        for row in data_rows:

            already_exist = str(row['bvid']) in listExistBVid
            ...
            if already_exist:
                modify_entry = ModifyEntry(
                    ...
                    ranks=get_ranks(row),
                    is_republish=None,
                    staff=None if pd.isna(row['staff']) else str(row['staff']),
                    is_examined=True,
                )
